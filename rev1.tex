...
usage and software crashes (which are manifestations of underlying defects). Once again, we are trying to build the best model explaining the number of exceptions, but we focus on discovering the relationship between exceptions and other post-deployment measures.
...
In summary, the goal of this study is not to make the best possible model for software crashes (exceptions), but to investigate the interrelationship between software usage and software faults (crashes, issues) and deriving a Quality measure that is independent of software usage. The primary contributions include an observed strong
...
The rest of the paper is organized as follows: In Section~\ref{s:data}, we describe the data used in our study, providing details of the data source, data collection process, and detailed data preprocessing steps.
...
It is worth mentioning that we had no control over which variables to measure during data collection, however,
while the set of variables we obtained are not exhaustive, we believe the three usage related variables: number of new users, usage intensity, and usage frequency adequately capture and report how much usage a software is getting. 
...
We only looked at the NPM packages that have more than 
10,000 downloads a month. According to~\cite{npmdl}, 
automated downloads are expected to be
...
the structure learning methods, and in some cases where the data distribution is too skewed to fit the normality assumption, discretizing the data produces better models than continuous data, so we considered it as a possibility as well.
...
\subsection{Explaining Exceptions for the Commercial Software}\label{s:explain}
...
We were doubtful about the accuracy of the model since some of the predictors had a moderate to high degree of correlation. We checked the Variance Inflation Factor~\cite{menard1995applied} and found it be less than 5 for all predictors, which shows the effect of multicollinearity isn't very strong. However, we also wanted to verify other assumptions behind the OLS model. We went through the list of model assumptions provided in~\cite{ols}, and found that our model does not satisfy the Normality assumption (assumption 6 in~\cite{ols}) , i.e. the residuals are not normally distributed, nor does it satisfy the criteria that the variance of the errors term is constant, conditional on the all predictors (assumption 5 in~\cite{ols}). However, it satisfies assumptions 1-3 (linearity, random sample, and no perfect multicollinearity  assumptions) and the weaker assumption 4 (namely that: $(1)\ E($residuals$) = 0$ and $(2) Cov($each predictor, residuals$) = 0$), as mentioned in~\cite{ols}. Therefore, according to ~\cite{ols}, the OLS estimator we obtained is not BLUE (Best Linear Unbiased Estimator), but it is unbiased, and consistent.
...
\subsection{Application of our model: A derived measure of Quality}\label{s:qual}
...
Our models show that the number of new users is the most important variable
for predicting the number of exceptions. However, they also show that 
some of the variance in exceptions remain unexplained using these predictors.
This result is not surprising, since we did not consider any code related metrics
like code complexity which could have an impact on the number of exceptions.
However, the lack of code complexity measures in our models do not undermine 
our result, because of the following reasons: \\(1) The goal of our study is studying the relationship 
...
projects, since the users might be more willing to work with a simpler code which is easier to tweak, for a closed-source scenario such as the one we presented in this study, such a relationship is practically impossible, since the users never see the source code.
...
analysis is reported in Appendix I. 
...
\section*{APPENDIX I: Exploring the Effect of Source Code Complexity on Exceptions for the Mobile Applications}
